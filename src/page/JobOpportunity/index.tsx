...
function JobOpportunity() {
    ...
    const { data: listJob, run: runGetJob } = useRequest(async () => {
        try {
            const queryAddress = `&addressId=${addressId}`
            const queryWork = `&workGroupId=${workGroupId}`
            const res = await axios.get(
                `http://localhost:3001/api/job?${workGroupId !== '' ? queryWork : ''}${
                    addressId !== '' ? queryAddress : ''
                }`
            )
            return res.data
        } catch (error) {}
                    ...
                    <div className='lg:col-span-3 col-span-full border-b border-b-gray-400 md:border-b-0 md:pb-0 pb-10'>
                        <div className='md:border-e-[2px] border-e-0 border-solid border-e-default'>
                            <div className='mb-8'>
                                ...
                                <Filter options={workGroups} jobAll='1' check={workGroupId} setCheck={setWorkGroupId} />
                            </div>
                            <div className='mb-8'>
